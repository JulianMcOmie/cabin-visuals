...
- `npm run test:visual` — the node test suite (core, core/visual, visualCopies, directors, photo, **instruments**, store, utils, persistence). Tests are colocated `*.test.ts` run with `node --test` + tsx; no jest/vitest. 753 pass, none red (verified 2026-08-02) — a new instrument's colocated test runs in the normal suite with no extra step.
