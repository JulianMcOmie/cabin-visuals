...
- `npm run test:visual` — the whole node test suite (core/visual, visualCopies, directors, photo, store, utils, persistence). Tests are colocated `*.test.ts` run with `node --test` + tsx; no jest/vitest.
