...
const spatialIds = ['symmetricMotion', 'symmetricRotation']
...
function makeCases(): FrameCase[] {
  const layout = () => entry('grid', { rows: 5, columns: 7, depth: 3, spacing: .7 })
  const tail = () => entry('radial', { copies: 4, radius: .12, plane: 1, tilt: 17 })
  const cases: FrameCase[] = []
  const make = (name: string, chain: MoverOrSplitter[], beat: number, changes: Partial<FrameCase> = {}): FrameCase => ({
    name, chain, beat, placement: identity, objectOpacity: .65, size: .065, glow: .2, color: rgbColor, ...changes,
  })
  for (const kind of ['mirror', 'radial', 'axial'] as const) {
    const chain = [layout(), symmetric(kind), tail()]
    cases.push(make(`${kind}-sibling`, chain, .7), make(`${kind}-later`, chain, 2.1),
      make(`${kind}-negative-seek`, chain, -.5), make(`${kind}-seek-restored`, chain, .7, { repeat: `${kind}-sibling` }))
  }
  for (const plane of [0, 1, 2]) cases.push(make(`radial-plane-${plane}`, [layout(), symmetric('radial', { plane }), tail()], 1.1))
  for (const falloff of [0, 1, 2, 3]) cases.push(make(`axial-falloff-${falloff}`, [layout(), symmetric('axial', { falloff, anchor: falloff % 2 }), tail()], .9))
  ...
  cases.push(make('nested-then-operation', [nested, symmetric('radial'), tail()], .8))
  cases.push(make('cpu-prefix-unfolded-internal', [nested, cpuSeed(1)[1], tail()], .8, { size: .17, objectOpacity: 2.5 }))
  const countSeed = [entry('radial', { copies: 12, radius: 3, plane: 0 }, [note(1, 39), note(3, 47)]), cpuSeed(1)[1], tail()]
  cases.push(make('cpu-prefix-full-count', countSeed, 0, { size: .16, objectOpacity: 2.5 }),
    make('cpu-prefix-small-count', countSeed, 2, { size: .16, objectOpacity: 2.5 }),
    make('cpu-prefix-count-restored', countSeed, 0, { size: .16, objectOpacity: 2.5, repeat: 'cpu-prefix-full-count' }))
  const rich = [...cpuSeed(12), tail(), sharedLocalLayout({ transforms: [new Matrix4()], hueShifts: [.27] })]
  const richLarge = [...cpuSeed(513), tail(), symmetric('axial', { twist: 7, fold: 5, roll: 3 })]
  cases.push(make('cpu-rich-colors', rich, .7, { size: .16, objectOpacity: 2.5, glow: .8 }),
    make('plain-after-rich', [layout(), tail()], .7),
    make('rich-grow-texture-rows', richLarge, .7, { size: .038, objectOpacity: 2.5 }),
    make('operation-after-rich', [layout(), symmetric('mirror'), tail()], 1.1),
    make('cpu-rich-restored', rich, .7, { size: .16, objectOpacity: 2.5, glow: .8, repeat: 'cpu-rich-colors' }),
    make('cpu-rich-seek', rich, -.5, { size: .16, objectOpacity: 2.5, glow: .8 }),
    make('cpu-rich-placement', rich, .7, { placement: scaled, size: .12, objectOpacity: 2.5 }),
    make('cpu-rich-placement-restored', rich, .7, { size: .16, objectOpacity: 2.5, glow: .8, repeat: 'cpu-rich-colors' }),
    make('rich-grow-restored', richLarge, .7, { size: .038, objectOpacity: 2.5, repeat: 'rich-grow-texture-rows' }))
  return cases
}
...
function performanceCases() {
  ...
  return [
    { name: '32k-sibling-symmetric-motion', count: 32 ** 3, chain: [...radials(), symmetric('radial')] },
    { name: '32k-sibling-symmetric-rotation', count: 32 ** 3, chain: [...radials(), symmetric('axial')] },
    { name: 'million-sibling-symmetric-motion', count: 32 ** 4, chain: [...radials(), entry('radial', { copies: 32, radius: .05 }), symmetric('radial')] },
    { name: 'million-sibling-symmetric-rotation', count: 32 ** 4, chain: [...radials(), entry('radial', { copies: 32, radius: .05 }), symmetric('axial')] },
    { name: 'million-cpu-rich-prefix', count: 32 ** 4, chain: [...cpuSeed(32), ...radials()] },
  ]
}
