...
import type { MoverOrSplitter, VisualCopy } from '../../src/editor/core/visualCopies/types'
import type { ResolvedNote } from '../../src/editor/core/visual/types'

const spatialIds = ['fluidImpact']
const note = (beat: number, pitch: number, durationBeats = 2): ResolvedNote => ({ beat, pitch, durationBeats, velocity: 1, blockStartBeat: 0, blockEndBeat: 32 })
...
// This deliberately publishes no compact metadata: the production compiler
// must evaluate a bounded CPU prefix, preserving every color channel, then
// factor the remaining splitters. Index-dependent movement verifies the seed
// matrices too; the captured prefix is not merely a color lookup fixture.
function cpuSeed(copies: number): MoverOrSplitter[] {
  const colors: Partial<VisualCopy['colorShift']>[] = [
    { hue: .23, saturation: .2, lightness: -.08, tint: '#146cff', tintAmount: .45 },
    { hue: -.37, saturation: -.1, lightness: .06, tint: '#ffc314', tintAmount: .6, tintPerceptual: true },
    { hue: .41, saturation: .15, lightness: -.02, tint: '#ea1f73', tintAmount: .35, huePerceptual: true },
    { hue: -1.13, saturation: -.2, lightness: .07, tint: '#41efd4', tintAmount: .7, tintPerceptual: true, huePerceptual: true },
    { hue: .7, saturation: .8, lightness: .5, tint: '#a173ee', tintAmount: 1.4, tintPerceptual: true },
    { hue: -.2, saturation: -1.3, lightness: -.8, tint: '#de4973', tintAmount: -.3 },
    { hue: .1, saturation: .2, lightness: .05, tint: 'invalid', tintAmount: 1, huePerceptual: true },
    { hue: 0, saturation: .2, lightness: .1, tint: '#808080', tintAmount: 1 },
    { hue: .4, saturation: 0, lightness: 0, tint: '#808080', tintAmount: 1, huePerceptual: true },
    { hue: 0, saturation: 0, lightness: 0, tint: '#ff0000', tintAmount: 0, tintPerceptual: true },
    { hue: 0, saturation: 0, lightness: 0, tint: '#00ff00', tintAmount: 1, tintPerceptual: true },
    { hue: 1.01, saturation: -.1, lightness: .02, tint: '#0015ef', tintAmount: .5, tintPerceptual: true, huePerceptual: true },
  ]
  const columns = copies > 32 ? 27 : 4, spacing = copies > 32 ? .28 : 1.5
  const layout = sharedLocalLayout({ transforms: Array.from({ length: copies }, (_, i) =>
    new Matrix4().makeTranslation((i % columns - (columns - 1) / 2) * spacing,
      (Math.floor(i / columns) - (Math.ceil(copies / columns) - 1) / 2) * spacing, 0)) })
  const unknown: MoverOrSplitter = { maxOutputCount: 1, apply(copy, { index, beat, placementTransform }) {
    return [{ ...copy, transform: copy.transform.clone().multiply(new Matrix4().makeTranslation(
      Math.sin(beat + index * .17) * .08 + ((placementTransform?.elements[0] ?? 1) - 1) * .09,
      Math.cos(beat + index * .13) * .05, 0)),
    opacity: copy.opacity * [0, .0004, .02, .4, .8, 1][index % 6] * (.75 + .25 * Math.cos(beat)),
    colorShift: { ...copy.colorShift, ...colors[index % colors.length] } }]
  } }
  return [layout, unknown]
}
function symmetric(kind: 'mirror' | 'radial' | 'axial', changes: Record<string, number> = {}) {
  if (kind === 'axial') return entry('symmetricRotation', {
    mode: 2, drive: 0, axis: 2, axisYaw: 17, axisPitch: -13, centerX: .2, centerY: -.3, centerZ: .1,
    twist: 23, fold: 19, roll: 11, falloff: 2, span: 3, curve: 1.3, anchor: 0, ...changes,
  })
  return entry('symmetricMotion', { symmetry: kind === 'mirror' ? 1 : 0, motion: 1,
    plane: 3, distance: .45, angle: 17, beats: 2, ...changes },
  [note(0, 60, 8), note(0, 62, 8), ...(kind === 'mirror' ? [note(0, 65, 8)] : [])])
}
function fluid(changes: Record<string, number> = {}, notes: ResolvedNote[] = [note(0, 60), { ...note(.5, 60), velocity: .7 }]) {
  return entry('fluidImpact', { strength: 1.2, radius: 4, ...changes }, notes)
}
function makeCases(): FrameCase[] {
  const layout = () => entry('grid', { rows: 9, columns: 13, depth: 3, spacing: .45 })
  const tail = () => entry('radial', { copies: 3, radius: .09, plane: 1, tilt: 17 })
  const cases: FrameCase[] = []
  const make = (name: string, chain: MoverOrSplitter[], beat: number, changes: Partial<FrameCase> = {}): FrameCase => ({
    name, chain, beat, placement: identity, objectOpacity: .65, size: .045, glow: .2, color: rgbColor, ...changes,
  })
  const chain = [layout(), fluid(), tail()]
  for (const [name, beat] of [['before', -.5], ['onset', 0], ['attack', .2], ['overlap', .8], ['tail', 1.8], ['rest', 12]] as const) {
    cases.push(make(name, chain, beat))
  }
  cases.push(make('overlap-restored', chain, .8, { repeat: 'overlap' }))
  cases.push(make('placed', chain, .8, { placement: scaled }))
  const nested = splitterWithChildChain(entry('radial', { copies: 7, radius: 1.2, plane: 1 }),
    [entry('mover', { motion: 1, mode: 1, angleX: 17, angleY: 29, angleZ: 45 })])
  cases.push(make('nested-then-fluid', [nested, fluid(), tail()], .8))
  cases.push(make('fluid-between-nested', [nested, fluid(), nested, tail()], .8))
  cases.push(make('fluid-nested-child', [layout(), splitterWithChildChain(entry('radial', { copies: 5, radius: .3 }), [fluid()])], .8))
  const singular = sharedLocalLayout({ transforms: [new Matrix4(), new Matrix4().makeScale(0, 1, 1),
    new Matrix4().makeScale(1e-13, 1, 1), new Matrix4().makeScale(-.7, 1.2, .8)] })
  cases.push(make('singular-and-mirrored', [entry('radial', { copies: 5, radius: 1.3 }), singular, fluid(), nested], .8))
  const rich = [...cpuSeed(12), fluid(), tail(), sharedLocalLayout({ transforms: [new Matrix4()], hueShifts: [.27] })]
  cases.push(make('rich-seeds-and-fluid', rich, .8, { size: .12, objectOpacity: 2.5, glow: .8 }),
    make('plain-after-fluid', [layout(), tail()], .8),
    make('fluid-restored-after-plain', chain, .8, { repeat: 'overlap' }),
    make('fluid-after-symmetric', [layout(), symmetric('axial', { twist: 7, fold: 5, roll: 3 }), fluid(), tail()], .8))
  for (const axis of [0, 1, 2]) cases.push(make(`axis-${axis}`, [layout(), fluid({ axis, centerX: .3, centerY: -.2, centerZ: .1 }), tail()], .8))
  cases.push(make('pure-pressure', [layout(), fluid({ curl: 0, turbulence: 0 }), tail()], .2),
    make('pure-curl', [layout(), fluid({ strength: 2, curl: 2, turbulence: 0, rebound: 0 }), tail()], .8),
    make('small-eddies', [layout(), fluid({ eddySize: .1, turbulence: 2, curl: -2, flow: -2 }), tail()], .8),
    make('small-field', [layout(), fluid({ radius: .25, strength: 10 }), tail()], .2),
    make('slow-field', [layout(), fluid({ decay: 8, flow: -2 }), tail()], 4),
    make('full-strength-overlaps', [layout(), fluid({ strength: 10, curl: 2, turbulence: 2, rebound: 1, radius: 20 }), tail()], .8))
  return cases
}
...
function performanceCases() {
  const radials = (count = 32) => [entry('radial', { copies: count, radius: 2.1 }),
    entry('radial', { copies: count, radius: .6, plane: 1 }), entry('radial', { copies: count, radius: .17, plane: 2 })]
  // Repeated note onsets keep impacts alive throughout timing; the field is
  // deliberately AFTER fanout so a CPU prefix cannot disguise a fallback.
  const notes = Array.from({ length: 40 }, (_, i) => note(i * .5, 60))
  return [
    { name: '32k-fluid-after-splitters', count: 32 ** 3, chain: [...radials(), fluid({}, notes)] },
    { name: 'million-fluid-after-splitters', count: 32 ** 4, chain: [...radials(), entry('radial', { copies: 32, radius: .05 }), fluid({}, notes)] },
    { name: 'million-fluid-with-symmetric-rotation', count: 32 ** 4,
      chain: [...radials(), entry('radial', { copies: 32, radius: .05 }), symmetric('axial', { twist: 7, fold: 5, roll: 3 }), fluid({}, notes)] },
  ]
}
...
button.onclick = async () => {
  ...
  try {
    ...
    const probeLayout = entry('grid', { rows: 9, columns: 13, depth: 3, spacing: .45 })
    const resting = resolveVisualCopies([probeLayout], .8), moved = resolveVisualCopies([probeLayout, fluid()], .8)
    const distances = moved.map((copy, index) => new Vector3().setFromMatrixPosition(copy.transform)
      .distanceTo(new Vector3().setFromMatrixPosition(resting[index].transform)))
    const displaced = distances.filter(distance => distance > .01).length
    result.displacement = { count: distances.length, displaced, mean: distances.reduce((a, b) => a + b, 0) / distances.length,
      max: Math.max(...distances), pass: displaced > distances.length * .5 }
    ...
    result.pass = (result.displacement as { pass: boolean }).pass && frames.every(frame => frame.pass) && benchmarks.every(benchmark => benchmark.pass) && result.glError === 0 && errors.length === 0
