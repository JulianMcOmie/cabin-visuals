...
export function GradientPathEditor({ targetId, bound, curved, colorA, colorB }: { targetId: string; bound: UserInterfaceParameter; curved: boolean; colorA: string; colorB: string }) {
  ...
  return <div className="space-y-2 text-xs">
    <button className="rounded border border-[color-mix(in_srgb,var(--text)_20%,transparent)] px-2 py-1" aria-pressed={editing} onClick={()=>{ if(!editing) useUIStore.getState().setCanvasView('scene'); setEditing(!editing) }}>{editing?'Done editing on stage':'Edit on stage'}</button>
    <p className="text-[var(--text-3)]">World coordinates. Drag handles in XY; set Z below.{curved?' Click the stage path to add a point.':''}</p>
    <div className="flex gap-2">
      <select aria-label="Path point" value={index} onChange={e=>setSelected(Number(e.target.value))} className="bg-[var(--bg-panel)]">
    ...
    {(curved?['point','incoming','outgoing']:['point']).map(key=>{
      ...
      return <div key={kind} className="flex items-center gap-1"><span className="w-16 text-[var(--text-3)]">{kind}</span>{['X','Y','Z'].map((axis,k)=><label key={axis} className="min-w-0 flex-1 text-[var(--text-3)]">{axis}<input aria-label={`${kind} ${axis}`} type="number" step="0.1" value={Number(nodes[index][kind][k].toFixed(3))} className="w-full rounded bg-[color-mix(in_srgb,var(--text)_5%,transparent)] px-1 text-[var(--text)]" onChange={e=>{ if(e.target.value==='')return; const v=Number(e.target.value); if(!Number.isFinite(v)||Math.abs(v)>10000)return;const p=[...nodes[index][kind]] as PathPoint;p[k]=v;bound.setValue(JSON.stringify(moveGradientHandle(nodes,index,kind,p))) }}/></label>)}</div>
    })}
  </div>
}
