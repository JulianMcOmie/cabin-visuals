...
import { Color, PMREMGenerator, type Group, type MeshPhysicalMaterial } from 'three'
...
function PreviewSolid({ geometry, tube, sides, color, surface, size, dims, spinSpeed }: {
...
function SolidPreview({ geometry, tube, sides, color, surface, size, dims, spinSpeed }: {
  geometry: FundamentalGeometryId
  tube: number
  sides: number
  color: string
  ...
  return (
    ...
    >
      <Canvas dpr={[1, 1.5]} camera={{ position: [0, 1.1, 4.6], fov: 42 }} gl={{ antialias: true, alpha: true }} shadows>
        ...
        <PreviewSolid geometry={geometry} tube={tube} sides={sides} color={color} surface={surface} size={size} dims={dims} spinSpeed={spinSpeed} />
