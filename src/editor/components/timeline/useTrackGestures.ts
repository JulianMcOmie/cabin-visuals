...
function canPasteChildUnder(track: Track, copiedRoot: Track): boolean {
  ...
  if (copiedRoot.type === 'automation') return track.type === 'base' || track.type === 'mover' || track.type === 'director'
...
export function useTrackGestures({ laneRef, dragGuideRef }: UseTrackGesturesOptions) {
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      // Cmd/Ctrl+R is deliberately NOT bound (it used to be Logic-style Repeat):
      // it is the browser's refresh, and hijacking it meant a reflexive reload
      // with blocks selected silently edited the project instead.
