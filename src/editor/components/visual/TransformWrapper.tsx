...
function SingleTransform({ instance, children }: { instance: PluginInstance; children: ReactNode }) {
 ...
 */
export function TransformWrapper({ plugins, children }: { plugins: PluginInstance[]; children: ReactNode }) {
  const transforms = plugins.filter((i) => getPlugin(i.pluginId)?.category === 'transform')
