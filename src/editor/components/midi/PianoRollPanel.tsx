...
export function PianoRollPanel() {
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      const currentEditingBlock = useUIStore.getState().editingBlock
      if (!currentEditingBlock) return
      const { currentBeat } = useTimeStore.getState()
      const rightBlockIds = useProjectStore.getState().splitBlocksAtBeat(new Set([currentEditingBlock.blockId]), currentBeat)
      if (rightBlockIds.length === 0) return
      e.preventDefault()
      setSelectedBlockIds(new Set([currentEditingBlock.blockId, ...rightBlockIds]))
    }
    document.addEventListener('keydown', onKeyDown)
    return () => document.removeEventListener('keydown', onKeyDown)
  }, [setSelectedBlockIds])
