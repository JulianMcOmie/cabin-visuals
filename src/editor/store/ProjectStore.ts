...
interface ProjectState {
  ...
  splitBlocksAtBeat: (blockIds: Set<string>, beat: number) => string[]
...
export const useProjectStore = create<ProjectState>((set) => ({
  ...
  splitBlocksAtBeat: (blockIds, beat) => {
    ...
    set((s) => {
      if (blockIds.size === 0) return s
      let changed = false
      const tracks: Record<string, Track> = {}
      const nextCreatedIds: string[] = []
      for (const [trackId, track] of Object.entries(s.tracks)) {
        ...
        for (const block of track.blocks) {
          ...
          const split = splitBlockAtBeat(block, beat, s.beatsPerBar)
          if (!split) {
            blocks.push(block)
            continue
          }
          blocks.push(split.left, split.right)
          nextCreatedIds.push(split.right.id)
          changed = true
          trackChanged = true
        }
        tracks[trackId] = trackChanged ? { ...track, blocks } : track
      }
      if (!changed) return s
