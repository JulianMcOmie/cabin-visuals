...
- **Per-COPY time (2026-08-24, position routing 2026-08-27): the chain kernel shifts `context.beat` per copy for pattern entries under a Stagger (`visualCopies/resolveVisualCopies.ts`'s time channel), and this engine builds PER-COPY OBJECT STATES on top** — for a track whose chain declares `emitsCopyClocks` (structural, never a probe: a triggered Stagger emits nothing before its first note), `computeAtBeat` re-runs the whole per-object assembly at `objBeat − offset` per shifted copy: params, energy, active notes, effect lanes, the instrument's own `localTransform`/world. `getObjectState(trackId, visualCopyIndex)` serves them; `useInstrumentFrame`, `ObjectRenderer` and `ShaderWrapper` pass their occurrence index, and the instanced fast path stands down for staggered tracks (`isTrackStaggered`). **CHILD POSITION routes every clock**: a lane or mover ABOVE the Stagger is the pattern (copy clock), one AFTER it is a live overlay (real timeline) — `orderEmitterClocks` in resolve.ts moves emitters to the chain front and stamps entries/lanes with `clockSkipEmitters`; `computeCopyStates` subtracts the skipped emitters' offsets per lane via the kernel's per-copy checkpoints (`copyClockShift`). The old hardcoded spatial-tf-stays-object-clocked exception is subsumed (an overlay spatial lane is below every chain child, hence below the emitter); only the switcher gate stays unconditionally object-clocked. One resolve.ts consequence: `resolveOwnMoverOrSplitter`'s automation wrapper must forward `applyFramed` AND `emitsCopyClocks` (it does), or automating an emitter's knobs silently strips every copy's clock.

## resolve.ts

Flattens each scene's track forest depth-first (cycle-guarded), expands looped blocks' notes (`noteFlatten.ts`), gathers child lanes per object: `automation` (in one of its four modes, below), `ability`, effect-automation (`fx:<instanceId>:<key>` targets, parsed by `effects/automation.ts`), and the track's mover/splitter chain. `ProjectSnapshot` is a structural slice of ProjectStore — the engine never imports store internals.

`noteFlatten.ts` repeats only notes inside the source loop window; onsets at/past its end are excluded and crossing tails are clipped at each seam. The authored notes remain untouched so unlooping/resizing can recover them. Negative phases still fold for legacy split blocks. Empty windows return immediately, including windows whose only notes are outside. Timeline previews, MIDI ghosts, activity, and playback share this tiler.
...
- `wordFormation.ts` — Word Formation lanes: the geometry a text instrument seats its
  ...
  - **Geometry is knob-driven, so `syncParams` refreshes it at 60fps** like the instrument
    params — and it REPLACES the lane array rather than mutating it, because
