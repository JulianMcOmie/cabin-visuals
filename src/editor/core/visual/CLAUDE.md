...
- **Per-COPY time (2026-08-24): the chain kernel shifts `context.beat` per copy for entries below a Stagger (`visualCopies/resolveVisualCopies.ts`'s time channel), and this engine builds PER-COPY OBJECT STATES on top** — for a track whose chain declares `emitsCopyClocks` (structural, never a probe: a triggered Stagger emits nothing before its first note), `computeAtBeat` re-runs the whole per-object assembly at `objBeat − offset` per shifted copy: params, energy, active notes, envelopes, effect lanes, the instrument's own `localTransform`/world. `getObjectState(trackId, visualCopyIndex)` serves them; `useInstrumentFrame`, `ObjectRenderer` and `ShaderWrapper` pass their occurrence index, and the instanced fast path stands down for staggered tracks (`isTrackStaggered`). Two things deliberately stay OBJECT-clocked inside a copy state: the SPATIAL tf lanes (their per-copy half is the chain weave — position semantics — and a below-the-chain lane sampled per copy would tear the formation-as-one placement apart) and the switcher gate. `tfOpacity` and instrument-param lanes ride the copy clock — that is how "everything above the Stagger staggers" without any lane-specific weave. One resolve.ts consequence: `resolveOwnMoverOrSplitter`'s automation wrapper must forward `applyFramed` AND `emitsCopyClocks` (it does), or automating an emitter's knobs silently strips every copy's clock.
