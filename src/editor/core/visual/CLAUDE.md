...
- **Per-COPY time is the chain kernel's job, not this engine's**: a Stagger entry shifts `context.beat` per copy for the entries below it (`visualCopies/resolveVisualCopies.ts`'s time channel — see that module's guide). Nothing here changes: `computeAtBeat` still hands the kernel one `objBeat`, instrument state/energy/envelopes stay object-wide, and the object-level automation overlay is untouched. One resolve.ts consequence: `resolveOwnMoverOrSplitter`'s automation wrapper must forward `applyFramed` (it does), or automating an emitter's knobs silently strips every copy's clock.
