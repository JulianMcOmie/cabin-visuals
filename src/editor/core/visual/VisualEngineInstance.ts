...
export function createVisualEngine() {
  ...
  function setProject(input: ProjectState | ProjectSnapshot) {
   ...
   */
  function syncParams(input: ProjectState | ProjectSnapshot) {
    const p = normalizeProject(input)
    project = p
    bpm = p.bpm
  ...
  function getSceneBackdrop(sceneId: string): SceneBackdrop | undefined {
  ...
  function getSceneFxOverrides(sceneId: string): Record<string, Record<string, number>> | undefined {
  ...
  function setPreviewObjectState(trackId: string, state: ObjectState | null): void {
  ...
  function getObjectState(trackId: string, visualCopyIndex?: number): ObjectState | undefined {
   ...
   *  copies). */
  function isTrackStaggered(trackId: string): boolean {
  ...
  function getCompositionLayers(): CompositionLayer[] {
    return compositionLayers
  }

  function setMainCompositionOverride(value: boolean) {
  ...
  function setMainPreviewEnabled(value: boolean) {
  ...
  function setEditorPreviewSceneId(sceneId: string | null) {
  ...
  function setMountedRenderScenes(scenes: Map<string, ThreeScene>) {
    mountedRenderScenes = scenes
  }

  function getMountedRenderScenes(): Map<string, ThreeScene> {
  ...
  function getPeakVisualCopyOpacity(trackId: string): number {
  ...
  function subscribeObjects(cb: () => void) {
    listeners.add(cb)
    return () => {
      listeners.delete(cb)
    }
