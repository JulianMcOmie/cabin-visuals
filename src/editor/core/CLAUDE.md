...
`sceneRowColor.ts` is the row-color convention for rows that MEAN a scene (sceneSwitcher's; Cut/Radial Cut deliberately still cycle): the row wears its scene's **backdrop hue** re-voiced at the note palette's lightness/chroma, so the piano roll reads like the frames it cuts to. Only the hue crosses over — a black room's notes would otherwise be invisible — a gradient backdrop is its two stops averaged in OKLab (chroma-weighted, so a navy→black fade still reads navy), and a backdrop with no hue at all (default black, transparent) falls back to the caller's cycle color so the rows still tell each other apart.
