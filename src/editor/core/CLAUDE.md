...
Director params are automatable like any other track's: `directorAutomatableParams(def)` (types.ts: the shared Opacity + the def's params) is the single source the context menu, the lane's piano-roll rows, and the engine all read. Directors never enter the resolved graph, so their lanes are gathered + sampled per frame inside `VisualEngine.resolveComposition` (a pure function of the beat, like the note flattening the defs already do each frame) — a new director's params get automation with no extra wiring, and its `resolve()` sees the sampled values through the same `track.params` reads it already does.
