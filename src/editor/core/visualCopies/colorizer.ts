// The Colorizer: every note is a COLOR EVENT.
//
// One note, one flash: the object's colors are pulled toward a color you pick,
// by an amount an envelope shapes. Short notes with no attack and a fast
// release are a percussive hit synced to a snare's amplitude curve; a held note
// with a slow attack is a sustained wash. There is no mode switch, because the
...
/** The one row this definition answers. Everything the Colorizer does today is
 *  a single function - flash - so it declares a single note rather than a
 *  keyboard the user has to guess the meaning of. */
...
const COLORIZER_ROWS: MidiRowDef[] = [{ pitch: COLORIZER_FLASH_PITCH, label: 'Color flash' }]
...
const NO_OUTPUT: ColorizerOutput = { tintAmount: 0, envelope: 0 }
...
export function evaluateColorizer(
  ...
  for (const note of notes) {
    if (note.pitch !== COLORIZER_FLASH_PITCH) continue
  ...
  if (gainPeak <= 0) return NO_OUTPUT
  ...
  resolve({ settings, notes }) {
    return {
      apply(visualCopy, { beat, index }) {
        const output = evaluateColorizer(notes, settings, beat, index)
        return [{
