...
- Individual movers/splitters: `rotationMovers`, `translationOscillator`, `burst*`, `radialMotion`, `visibility`, `grid`, `polyhedron`, `tunnel`, `parametricPattern`, `waveTerrain`, `forceFieldPush`, `colorizer` (note → envelope-shaped flash toward an absolute color), `consolidatedMover`, `meteorImpact`, `conveyor` (held note = belt running, six directions; loops each copy through a field box and fades it at the faces), `splitterMidi` (MIDI gating), `motionBasis`/`motion` (shared math), `burstEasings`.
...
- **Two kinds of wrapping, and they look nothing alike.** Motion's bounds wrap the mover's own accumulated TRANSLATION, so every copy shares one offset and the whole formation jumps home together. Conveyor wraps each copy's own POSITION (read off the incoming `transform`'s translation), so copies cross the face one at a time and the formation reads as an endless belt. A per-copy wrap has to PRE-multiply its delta: the loop planes are fixed, so the fold has to be measured in the frame the box lives in, and a rotation above the mover must not turn it. Layouts want to fill the box — a Grid whose extent exceeds `2 × span` gets folded onto itself and copies overlap.
