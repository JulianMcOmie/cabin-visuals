...
- `countLane.ts` — **the shared COUNT lane**, the default MIDI vocabulary on every
  splitter with a count knob (`radial`/`line` copies, `grid` rows, `symmetry`
  mirrors; 2026-08-25, retiring Radial's radius value lane, Grid's spacing value
  lane and Line/Symmetry's mute maps — Polyhedron keeps the mute map because its
  count comes from the shape geometry, and Tunnel/Approach/Pattern keep their
  bespoke vocabularies). A note's pitch names a whole-number count through the
  SAME integer row grid an automation lane on the param uses
  (`automationIntegerGrid`, one row per whole number from pitch 36 = min), and the
  count JUMPS at the onset and HOLDS until the next — a step function matching the
  `'step'` interpolation integer automation lanes default to — resting at the knob
  (or its automation) before the first onset; duration/velocity ignored, chords
  keep the largest, out-of-span pitches DROP (never clamp), which is what makes
  every retired vocabulary's notes degrade to the knob instead of misreading.
  Each sampled count is a TRUE re-layout (`resolveAt` is the definition's whole
  layout as a pure function of settings), so a note means exactly what turning the
  knob would; the copy-count invariant is kept the automated-settings way — the
  entry re-resolves per count (memoized per count, a cache not playback state) and
  publishes `structuralVariants` [max, min] bracketing the lane's reach, which
  `resolveOwnMoverOrSplitter` (core/visual/resolve.ts) FLATTENS through its own
  automation variants (the probe swaps variants one level deep only — skip that
  flatten and a mid-timeline jump to a large count overflows the pool). The count
  params themselves carry `integer: true` (NumberParamDef), which is also what
  makes a fresh automation lane on them start on the integer grid with stepped
  interpolation (store `addAutomationTrack`).
...
- **`mover` — THE motion mover (2026-08 consolidation).** One definition covering the
  ...
  constant movers and retargeting their automation lanes). All cells share the
...
stays radius. Its lane is the shared COUNT lane on COPIES (countLane.ts, 2026-08-25 —
it retired the 2026-08 radius value lane, whose swell grammar left with `valueLane.ts`;
RADIUS is knob + automation only now, and old radius notes above the count span, like
the mute pitches 122–127 before them, fall out of span and no-op). The polar options (2026-08-14) each default to neutral, so an untouched save is
...
- **GROWTH is a per-copy ratio on the RADIUS**, gated behind the SHAPE select
  ...
  but Line's is a ratio on the SIZE and this one is on the radius. Per copy, so a
  spiral keeps its proportions at whatever count the lane latches.
...
- **RINGS (2026-08-21) repeats the whole slot set outward**, with four
  ...
  directly. The lane still samples ONE count: every ring repeats that slot set, so a
  latched note resizes the whole stack and the ring offsets stay where the knobs put them.
...
- **The time shapes are the Mover's, not new ones.** Burst / Constant / Oscillate
  ...
  them is how it moves. New devices default to **Z / Uniform / Own center / Fold 45°**,
  with Twist and Roll zero: a bow in place about each copy's circle tangent.
  Along axis multiplies all channels by signed axial distance, so an XY wall
  around Z gets ZERO rotation regardless of Fold. Keep that explicit falloff's
  semantics; use Uniform for a flat formation. v20 persistence pins old implicit
  defaults, and the panel's Bow preset opts existing devices into the new pose.
  MIDI preview phrases must excite all three channels, or Fold appears dead in
  Oscillate / MIDI-only Constant even though the real evaluator works.
...
- **Empty lane: a free-running cycle** — DURATION is one copy's flight and COPIES
  ...
  `duration/copies` beats in copy order, every copy always in flight. Copies before
  their first birth start mid-flight with NEGATIVE births — evenly phased from beat 0,
  latching silence until the timeline reaches them. One formula, no start-up special
  case.
- **Spawn notes (pitch 60, the one MIDI row): each onset BIRTHS a copy — every onset,
  always.** A spawned copy flies for DURATION beats and then gates to opacity 0. The
  pool is SIZED BY THE PHRASE at resolve — first-fit over the flight intervals, growing
  a slot whenever every slot is mid-flight, which lands exactly at the peak overlap —
  so a dense phrase mounts a bigger pool rather than dropping notes or evicting
  flights (Duplicate Trail's precedent: a count DERIVED from resolved data is still
  beat-independent, and an automated DURATION brackets through `structuralVariants`
  since the count is monotonic in it). The COPIES knob is the loop's phase divider and
  is inert here. Note duration and velocity are ignored (impactPulse's rule). Opacity
  gating, never slot removal. `birthBeat` is the spawn note's onset, so a latching
  Colorizer hands each spawned copy the color sounding at the note you played.
  **LIFE = Endless makes every spawn permanent** — implemented as `Infinity` fed through
  the same allocator and flight check, so slots are never reused (one copy per note,
  the picture stacks), age runs on (a keyframe pattern holds its endpoint), and
  DURATION goes inert. Spawn-mode only: an empty lane always loops on DURATION.

Three more things to know:

- **Latching is note OWNERSHIP, not a frozen envelope.** The Colorizer's SAMPLE select
  (Live | Born) reads its lane at `birthBeat` with the gate `onset ≤ birth < note end`
  (half-open; zero-length notes own their onset instant) in place of the attack/release
  envelope. Freezing the envelope instead 50/50-blends every quantized sequence: a birth
  on a note boundary sits at the outgoing note's FULL release value (remaining = 1 at
  age == hold), and births land on note boundaries whenever the user quantizes — the
  common case, found by test, not a corner. Velocity still scales the latched strength;
  chords across rows still blend. No emitter above = Live fallback, never darkness.
