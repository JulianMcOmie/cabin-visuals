...
- **Scene**: a self-contained track forest (`tracks` record + `rootTrackIds`) with background color. The **Main scene** (`isMain`) is special: it holds *director* tracks that compose the other scenes into the final frame instead of object instruments.
