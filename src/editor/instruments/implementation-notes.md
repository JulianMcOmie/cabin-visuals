# Instrument implementation notes

Read only the section relevant to your task. For the ordinary development route,
use [Add an instrument](../../../docs/add-an-instrument.md). These notes preserve
feature-specific reasons and failure modes; linked source contracts own mechanics.
...
- **Held vs triggered is the axis that separates two warps.** BassRipple reads `activeNotes` and bends for as long as a note lasts; ImpactWarp reads note ONSETS out of the full stream and ignores duration entirely, summing an envelope per hit so a roll compounds. That is also what sorts them into the library's Rumble vs Impulse shelves. Impact Warp uses a C2-continuous attack (~100ms), broad recovery and one restrained rebound over 780ms, derived from beat age × secPerBeat. Its sole parameter is `impact`; legacy style/release/size values are ignored by the redesigned instrument.
...
- **Impact Warp preserves every screen symmetry** with a single centered isotropic sampling scale and one RGBA tap. Do not restore directional shoves, per-hit seeds or RGB separation. `impactDrive` softly bounds summed note envelopes, keeping retriggers continuous instead of clipping velocity at a hard limit; its negative side caps the rebound at 10% even for dense rolls. The panel and library card share its envelope, limiter and reach.
