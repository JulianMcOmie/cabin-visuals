...
Colocated `*.test.ts` here ARE run by `npm run test:visual` — the glob was widened when `KaleidoSolid.test.ts` landed, which also picked up earlier instrument tests that had been sitting unrun. All green as of 2026-07-30 (the "4 known-red BassRipple release tests" that older notes warned about are fixed); no separate command needed.
