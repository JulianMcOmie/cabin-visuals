...
Colocated tests here are NOT in `npm run test:visual`'s glob (see the root guide) —
`KaleidoSolid.test.ts` included. Widening the glob was tried and reverted: 4 BassRipple
release tests are currently red, so it would turn the whole suite red. Run them
explicitly after touching an instrument:
`node --import tsx --test src/editor/instruments/*.test.ts` (39 pass / 4 known-red).
