...
- `upgrade.ts` — `CURRENT_VERSION` (16 as of the Main→Composite scene rename; 15 was the Text Display clips redesign, 13 the mover consolidation, 12 the director de-specialization) + `UPGRADES[N]` steps, vN→vN+1. **Append-only; a shipped step is never edited.** Each step is pure. Bump the version whenever the document shape changes non-additively; purely additive fields can skip a bump if hydrate defaults them.
