...
- `upgrade.ts` — `CURRENT_VERSION` (17 as of lyric clips becoming notes; 16 was the Main→Composite scene rename) + `UPGRADES[N]` steps, vN→vN+1. **Append-only; a shipped step is never edited.** Each step is pure. Bump the version whenever the document shape changes non-additively; purely additive fields can skip a bump if hydrate defaults them.
